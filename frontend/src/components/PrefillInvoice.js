...
import { clearErrors, getSingleInvoice, newInvoice } from '../actions/Invoiceaction';
...
const PrefillInvoice = ({ match }) => {
    ...
    }, [dispatch, error, getInvoice, alert, invoiceId, history, success])
    return (
                ...
                loading ? <Loader /> :
                    <Fragment>
                        <div className='breadcrumb'>
                            <p><Link to='/invoice-list'>Quotation List</Link> <span><RightIcon /></span> <span>Prefill Create Quotation</span></p>
                        </div>
                        <h1 className='text-center head-text'>Prefi<span>ll Create Quo</span>tation</h1>
                        <MetaData title={`${getInvoice.companyname} - Recreate Quotation`} />
